        ...
        <div class="layui-container">
                    ...
                    <div class="layui-col-md12 jur-top jur-depart fl">
                        <div class="layui-form juf-form">
                            ...
                            <div class="layui-form-item">
                                ...
                                <div class="layui-inline">
                                    <div class="layui-input-inline">
                                        <input type="text" name="stop_time" class="layui-input" id="test-stop" placeholder="yyyy-MM-dd">
                                    </div>
                                </div>
                            </div>
                            <div class="layui-form-item">
                            ...
                            <div class="example-handle handle-add">
                                <?php if(checkAuth('/admin/user/adduser') == 'true'): ?>
                                <a class="examine-query" @click="importUsr">批量导入</a>
                                <a href="/admin/user/adduser" data-type="tab" title="新增账号" class="examine-query"></i>新增</a>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
            ...
            <table class="common-table" cellpadding="0" cellspacing="0" border="0" width="100%" align="center">
                ...
                <tr align="center" v-for="(item, index) in tableDataAll">
                    ...
                    <td>
                        <?php if(checkAuth('/admin/user/selectoneuser') == 'true'): ?>
                        <a class="audit-skip" data-type="tab" title="查看账号" v-bind:class="{ gary : item.status == 2 }" :href='item.status == 1 ? "/admin/user/selectoneuser?id=" + item.id : "javascript:;"'>查看</a>
                        <?php endif; if(checkAuth('/admin/user/edituser') == 'true'): ?>
                        <a class="audit-skip" data-type="tab" title="编辑账号" v-bind:class="{ gary : item.status == 2 }" :href='item.status == 1 ? "/admin/user/editUser?id=" + item.id : "javascript:;"'>编辑</a>
                        <?php endif; if(checkAuth('/admin/user/updatepassword') == 'true'): ?>
                        <a class="audit-skip" href="javascript:;" v-if="item.status == 1" @click="changePassword(index, item.id)">修改密码</a>
                        <a class="audit-skip" href="javascript:;" v-else  v-bind:class="{ gary : item.status == 2 }">修改密码</a>
                        <?php endif; if(checkAuth('/admin/user/adduserauth') == 'true'): ?>
                        <a class="audit-skip" data-type="tab" title="权限设置" v-bind:class="{ gary : item.status == 2 }" :href='item.status == 1 ? "/admin/user/adduserauth?id=" + item.id : "javascript:;"'>权限设置</a>
                        <?php endif; if(checkAuth('/admin/user/stopuser') == 'true'): ?>
                        <a class="audit-skip stop-skip" @click="accountStop(index, item.id, item.status)" href="javascript:;">{{item.status == 1 ? '停用' : '启用'}}</a>
                        <?php endif; if(checkAuth('/admin/user/deluser') == 'true'): ?>
                        <a class="audit-skip" v-if="item.status == 1" href="javascript:;" @click="accountDelete(item.id)" :data-id="item.id">删除</a>
                        <a class="audit-skip" href="javascript:;" v-else :data-id="item.id" v-bind:class="{ gary : item.status == 2 }">删除</a>
                        <?php endif; ?>
                    </td>
                </tr>
                </tbody>
...
<script src="__ADMINJS__/app/accountsManage.js"></script>
</body>
</html>