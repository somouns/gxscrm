...
class Auth
{
    ...
    public function check($name, $uid, $relation = 'or', $mode = 'url')
    {
        ...
        $rulelistall = Gfile::get("tp:allauthrule",300);
        if (!$rulelistall) {
            $rulelistall = Db::name('auth_rule')->field('name')->select();
            $rulelistall = array_column($rulelistall,'name');
            Gfile::set("tp:allauthrule", $rulelistall);
        }
        foreach ($rulelistall as $value){
            $ruleall[] = str_replace('_', '', $value);
        }
        if (!in_array($name,$ruleall)) {
            return true;
        }

        $where['id'] = array('in',$ruleids);
        $rulelist = Db::name('auth_rule')->field('name')->where($where)->select();
        $rulelist = array_column($rulelist,'name');
    ...
    public function getGroups($uid)
    {
        ...
        // 执行查询
        $normal = GroupConstant::GROUP_NORMAL;
        $user_groups = Db::view($auth_group_access, 'uid,group_id')
            ...
            ->select();
        //echo Db::view($auth_group_access, 'uid,group_id')->getLastSql();
    ...
    {
        if(empty($uid)) return [];
        static $_rulelist = []; //内存缓存
        if (isset($_rulelist[$uid])) return $_rulelist[$uid];
        ...
        //读取用户组所有权限规则
        $this->rules = Db::name($this->config['auth_rule'])->where($where)->field('id,condition,name')->select();
    ...
    public function getRuleIds($uid)
    {
        ...
        $surplus = $this->getAuthUser($uid);

        if(!empty($surplus)){
            array_push($groups, $surplus);
        }
        if(!empty($groups)){
            foreach ($groups as $key => $value) {
                if(isset($value['rules']) && !empty($value['rules'])){
                    $rules_temp=explode(',', $value['rules']);
                    foreach ($rules_temp as $key => $value) {
    ...
    public function getAuthUser($uid)
    {
        ...
        $rules = Db::table('crm_auth_employee')->field('employee_id,rules,datas')->where($where)->find();
        if(!empty($rules)){
            $result=$rules;
        }
        return $result;
